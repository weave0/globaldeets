...
(function () {
  ...
  const markets = [
    {
      ...
      languageProfile: { English: 95, Spanish: 13, Chinese: 3 },
      regulatory: ['SEC compliance', 'Insurance state regs'],
      business: ['Mature market', 'High competition'],
      capabilities: ['Multilingual support', 'RegTech advisory'],
    },
    {
      code: 'de',
      name: 'Germany',
      ...
      languageProfile: { German: 95, English: 65, Turkish: 4 },
      regulatory: ['BaFin supervision', 'GDPR'],
      business: ['Engineering strength', 'Export focus'],
      capabilities: ['Translation QA', 'Regulatory mapping'],
    },
    {
      code: 'jp',
      name: 'Japan',
      ...
      languageProfile: { Japanese: 98, English: 30, Korean: 0.5 },
      regulatory: ['FSA insurance', 'APPI privacy'],
      business: ['High tech adoption', 'Aging population'],
      capabilities: ['Localization pipeline', 'Language coverage'],
    },
  ];
  ...
  function renderOpportunityLists() {
    ...
    criticalList.innerHTML = markets
      .filter(m => languageGapSeverity[m.code] > 5)
      .map(m => `<li>${m.name} – severe language gap</li>`)
      .join('');
    highList.innerHTML = markets
      .filter(m => languageGapSeverity[m.code] > 3 && languageGapSeverity[m.code] <= 5)
      .map(m => `<li>${m.name} – moderate localization need</li>`)
      .join('');
  ...
  if (runAssessmentBtn) {
    runAssessmentBtn.addEventListener('click', () => {
      ...
      if (!marketCode) {
        if (window.showToast) window.showToast('Select a market', 'error');
        return;
      }
  ...
  if (roiBtn) {
    roiBtn.addEventListener('click', () => {
      ...
      if (!marketCode) {
        if (window.showToast) window.showToast('Select ROI market', 'error');
        return;
      }
      ...
      const complexityFactor =
        expansionType === 'new' ? 1.3 : expansionType === 'expansion' ? 1.1 : 1.2;
    ...
    try {
      new Chart(canvas, {
        ...
        data: {
          ...
          datasets: [
            {
              label: 'Language Gap Severity',
              data: markets.map(m => languageGapSeverity[m.code]),
  ...
  function renderLangChart() {
    ...
    try {
      new Chart(canvas, {
        type: 'doughnut',
        data: {
          labels: Object.keys(market.languageProfile),
          datasets: [
            {
              data: Object.values(market.languageProfile),
          ...
          datasets: [
            {
              ...
              data: [gain * 0.8, gain * 1.2, gain * 1.5],
              borderColor: '#10b981',
              backgroundColor: 'rgba(16,185,129,0.3)',
            },
            {
              label: 'Cumulative Investment',
              data: [investment, investment * 1.05, investment * 1.08],
              borderColor: '#ef4444',
  ...
  if (marketSelect) {
    marketSelect.addEventListener('change', () => {
      renderLangChart();
      if (window.showToast) window.showToast('Market changed', 'info');
    });
  }
