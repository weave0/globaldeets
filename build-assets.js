...
async function crop(src, dest, region, resizeTo) {
  ...
  if (resizeTo) pipeline = pipeline.resize(resizeTo, resizeTo, { fit: 'contain', background: { r: 0, g: 0, b: 0, alpha: 1 } });
  await pipeline.png().toFile(dest);
  console.log('✓', path.relative(__dirname, dest), `(${region.width}×${region.height}${resizeTo ? ` → ${resizeTo}px` : ''})`);
}
...
async function buildLogoGlobe() {
  ...
  // Globe mark occupies top ~62% of the 1254-px tall image (~778px), full width
  // Crop just the globe, then embed in a square black canvas for PWA icons
  const IMG_SIZE = 1254;
  const cropH = 790; // includes orbit arcs
  await sharp(src)
  ...
  await sharp(src)
    ...
    .png()
    .toFile(destHeader);
  console.log('✓ assets/logo-mark.png (GD globe mark, 1254×790, no padding)');
}
...
async function buildBannerAndLandscape() {
  console.log('\n── Banner & landscape ──');
  await copyNamed(
    path.join(ROOT, 'Banner with GD.png'),
    path.join(ROOT, 'og-banner.png')
  );
  await copyNamed(
...
// Standard grid: 8 rows × 5 cols
const COL_W = Math.floor(SHEET_W / 5);   // 280
const STD_ROW_H = 110;                    // rows 1–8 height in pixels
const BTM_Y = 8 * STD_ROW_H;             // y=880 where row 9 starts
const BTM_H = SHEET_H - BTM_Y;           // remaining height ~242
...
const BTM_CATEGORIES = [
  [41, 'logo-marks',     'Logo Marks & Symbols', { left: 0,               top: BTM_Y, width: Math.floor(SHEET_W / 2), height: BTM_H }],
  [42, 'favicon-styles', 'Favicon Styles',       { left: Math.floor(SHEET_W / 2), top: BTM_Y, width: SHEET_W - Math.floor(SHEET_W / 2), height: BTM_H }],
];
...
const ICON_LABEL_H = 22;   // px: category label area height
const ICON_COLS   = 5;     // icons per row within each category
const ICON_COL_W  = Math.floor(COL_W / ICON_COLS);           // ~56px
const ICON_ROW_H  = Math.floor((STD_ROW_H - ICON_LABEL_H) / 2); // ~44px
...
async function buildIconSheet() {
  ...
  for (const [name, sectionSlug, iconRow, iconCol] of SITE_ICONS) {
    ...
    if (!catRegion) { console.warn(`⚠ unknown section: ${sectionSlug}`); continue; }
