...
(function () {
  ...
  function activateFocusTrap(container) {
    ...
    if (!focusableEls.length) {
      focusableEls = [container.querySelector('.close')].filter(Boolean);
    }
  ...
  function focusByOffset(current, offset) {
    const idx = iconNavButtons.indexOf(current);
    if (idx === -1) return;
    const next = (idx + offset + iconNavButtons.length) % iconNavButtons.length;
    iconNavButtons[next].focus();
  }
  ...
  window.focusIconNav = index => {
    if (iconNavButtons[index]) iconNavButtons[index].focus();
  };
