...
(function () {
  ...
  class ParticleBackground {
    ...
    drawParticle(p) {
      ...
      this.ctx.shadowColor = `hsla(${p.hue},80%,60%,${p.opacity * 0.5})`;
      this.ctx.fill();
    ...
    connectParticles() {
      for (let i = 0; i < this.particles.length; i++) {
        for (let j = i + 1; j < this.particles.length; j++) {
          ...
          if (dist < 150) {
            const op = (1 - dist / 150) * 0.15;
            this.ctx.beginPath();
            this.ctx.strokeStyle = `rgba(59,130,246,${op})`;
  ...
  class ParallaxController {
    ...
    init() {
      this.addParallaxElement('.logo h1', 0.3);
      this.addParallaxElement('.stats-bar', 0.15);
      this.addParallaxElement('.project-card', 0.1);
      window.addEventListener('scroll', () => this.requestTick());
      this.update();
    }
    addParallaxElement(selector, speed) {
